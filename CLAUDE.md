...
```text
apps/web/      Web app — file list, editor shells, sync, embed (see apps/web/editors/)
...
lib/           Shared logger — lib/logger/ (core) + apps/web/lib + server/lib adapters
scripts/       Build tooling (_scripts/ = dev/deploy/desktop helpers, gitignored)
...
2. **App Development**: Work in `apps/web/` for Web-specific features
