...
const targets = [
  ...
  path.join(root, "apps/web/editors/mindmap/native/index.html"),
];

for (const file of targets) {
  if (!fs.existsSync(file)) {
    console.warn(
      `[normalize-mind-map-index] skip missing ${path.relative(root, file)}`,
    );
    continue;
  }
  ...
  if (after === before) {
    console.log(
      `[normalize-mind-map-index] ok (unchanged) ${path.relative(root, file)}`,
    );
  } else {
    fs.writeFileSync(file, after);
    console.log(
      `[normalize-mind-map-index] updated ${path.relative(root, file)}`,
    );
  }
}
