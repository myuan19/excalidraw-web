...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (arg === "--component") out.component = next();
    else if (arg === "--run") out.fields.run = next();
    else if (arg === "--case") out.fields.case = next();
    else if (arg === "--trace") out.fields.trace = next();
    else if (arg === "--request") out.fields.request = next();
    else if (arg === "--tab") out.fields.tab = next();
    else if (arg === "--file") {
      const value = next();
    ...
    } else if (arg === "--grep") out.grep = next();
...
function latestLogPath(logDir, kind) {
  const prefix = `${kind}-`;
  return listLogFiles(logDir).find((file) => file.name.startsWith(prefix))?.path;
}
...
function matchFileAliases(entry, expected) {
  ...
  return values.some((value) => String(value ?? "") === String(expected.fileId8));
}
