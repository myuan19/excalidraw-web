...
/**
 * @param {unknown} value
 * @param {{ maxStringLength: number, depth: number }} opts
 * @returns {unknown}
 */
...
/**
 * Old `logger.info("message", data)` calls do not yet provide stable events.
 * Keep them queryable under a predictable legacy namespace until call sites are
 * migrated to `logger.event(level, event, message, opts)`.
 *
 * @param {string} module
 * @returns {string}
 */
...
/**
 * Fixed-column Debug Event formatter. Transports can opt into this format
 * without changing logging call sites.
 *
...
export function formatDebugEvent(entry) {
  ...
  const base = `${entry.ts} | ${entry.level.toUpperCase()} | ${component} | ${sourceLocation} | ${contextColumn} | ${event} - ${message}`;
...
export class Logger {
   ...
   */
  constructor({ module, source, component, context, minLevel = "info", transports }) {
    this._module = module;
    this._source = source;
  ...
  /** @param {string} msg @param {unknown} [data] */
  trace(msg, data) { this._log("trace", msg, data); }
  /** @param {string} msg @param {unknown} [data] */
  ...
  /**
   * New Debug Event API. Prefer this for new instrumentation so event names are
   * stable and fields remain separate from the human message.
   *
  ...
  _emit(level, payload) {
    ...
    if (LEVEL_VALUE[level] < (LEVEL_VALUE[minLevel] ?? 1)) return;
