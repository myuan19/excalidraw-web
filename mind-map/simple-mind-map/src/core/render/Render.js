...
class Render {
  ...
  handlePaste(event) {
    ...
    console.log('[DEBUG] mindmap-paste | handlePaste enter', {
    ...
    if (target && target !== document.body) {
      ...
      if (!isEditNode && isWritableTarget) {
        console.log('[DEBUG] mindmap-paste | return writable target')
        return
      }
    }
    const clipboardData =
    ...
    if (!clipboardData || !clipboardData.items) {
      console.log('[DEBUG] mindmap-paste | return missing clipboard items')
      return
    }
    ...
    console.log('[DEBUG] mindmap-paste | clipboard snapshot', {
      hasText: !!text,
  ...
  async paste(clipboardSnapshot = null) {
    ...
    if (!disabledClipboard && (clipboardSnapshot || checkClipboardReadEnable())) {
      try {
        ...
        console.log('[DEBUG] mindmap-paste | paste resolved clipboard', {
        ...
        if (img && (!text || !onlyPasteTextWhenHasImgAndText)) {
          try {
            ...
            if (this.activeNodeList.length > 0) {
              ...
              console.log('[DEBUG] mindmap-paste | SET_NODE_IMAGE done', {
                activeNodeCount: this.activeNodeList.length,
                width: imgData.size.width,
                height: imgData.size.height
              })
            ...
            console.log('[DEBUG] mindmap-paste | image paste failed', error)
            errorHandler(ERROR_TYPES.LOAD_CLIPBOARD_IMAGE_ERROR, error)
          }
        }
      } catch (error) {
        console.log('[DEBUG] mindmap-paste | read clipboard failed', error)
