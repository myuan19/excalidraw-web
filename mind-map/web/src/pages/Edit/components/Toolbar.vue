<template>
  <div class="toolbarContainer" :class="{ isDark: isDark }">
    <div class="toolbar" ref="toolbarRef">
      ...
      <div
        class="toolbarBlock middleToolbarBlock"
        v-if="isHostMode || fileTreeVisible"
      >
        <div
          class="toolbarBtn aiToolbarBtn"
          v-if="isHostMode"
          @click="$bus.$emit('ai_create_all')"
        >
      ...
      <!-- 宿主系统操作 -->
      <div class="toolbarBlock hostToolbarBlock" v-if="isHostMode">
        <div class="toolbarBtn" @click="backToFiles">
          <span class="icon iconfont iconhoutui-shi"></span>
          <span class="text">{{ $t('toolbar.hostBackToFiles') }}</span>
        </div>
        <div class="toolbarBtn" @click="requestSave">
          <span class="icon iconfont iconlingcunwei"></span>
          <span class="text">{{ $t('toolbar.hostSave') }}</span>
        </div>
        <div class="toolbarBtn" @click="openHistory">
          <span class="icon iconfont iconzuijinliulan"></span>
          <span class="text">{{ $t('toolbar.hostHistory') }}</span>
        </div>
        <div class="hostSaveHint" v-if="hostSaveStatusText">
          {{ hostSaveStatusText }}
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    hostSaveStatusText() {
      if (this.hostSaveStatus.saving) {
        return '保存中…'
      }
      return this.hostSaveStatus.hint
    }
  },
  watch: {
  ...
  created() {
    ...
    this.$bus.$on('host_save_status', this.onHostSaveStatus)
  },
  mounted() {
    this.computeToolbarShow()
  ...
  beforeDestroy() {
    ...
    this.$bus.$off('host_save_status', this.onHostSaveStatus)
