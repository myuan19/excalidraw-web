...
const debugMindMapOpen = (label, data = {}) => {
  console.log('[DEBUG] mindmap-open | vue Edit ' + label, {
...
const EMBED_PREVIEW_TARGET_ROOT_SCREEN_RATIO =
  EMBED_PREVIEW_BASELINE_ROOT_SCREEN_RATIO *
  EMBED_PREVIEW_ROOT_SCREEN_RATIO_MULTIPLIER
const EMBED_PREVIEW_BASELINE_NODE_COUNT = previewViewportConfig.baselineNodeCount
const EMBED_PREVIEW_MIN_VISUAL_SCALE_NODE_COUNT =
  previewViewportConfig.minVisualScaleNodeCount
const EMBED_PREVIEW_SINGLE_NODE_VISUAL_SCALE =
  previewViewportConfig.singleNodeVisualScale
const EMBED_PREVIEW_MIN_NODE_VISUAL_SCALE =
  previewViewportConfig.minNodeVisualScale
const EMBED_PREVIEW_CENTER_TOWARD_OTHERS_RATIO =
  previewViewportConfig.centerTowardOthersRatio
const EMBED_PREVIEW_ROOT_CENTER_LIMIT_RATIO =
  previewViewportConfig.rootCenterLimitRatio

const clampEmbedPreviewValue = (value, min, max) => {
  if (max < min) return min
  return Math.min(Math.max(value, min), max)
}

const centerOfEmbedPreviewBounds = bounds => {
  return {
...
const unionEmbedPreviewBounds = items => {
...
const computeEmbedPreviewNodeVisualScale = nodeCount => {
  if (nodeCount <= EMBED_PREVIEW_BASELINE_NODE_COUNT) {
    const t =
      (EMBED_PREVIEW_BASELINE_NODE_COUNT - nodeCount) /
      (EMBED_PREVIEW_BASELINE_NODE_COUNT - 1)
    return (
      1 +
      clampEmbedPreviewValue(t, 0, 1) *
        (EMBED_PREVIEW_SINGLE_NODE_VISUAL_SCALE - 1)
    )
  }
  const t =
    (nodeCount - EMBED_PREVIEW_BASELINE_NODE_COUNT) /
    (EMBED_PREVIEW_MIN_VISUAL_SCALE_NODE_COUNT -
      EMBED_PREVIEW_BASELINE_NODE_COUNT)
  return (
    1 -
    clampEmbedPreviewValue(t, 0, 1) *
      (1 - EMBED_PREVIEW_MIN_NODE_VISUAL_SCALE)
  )
}
...
export default {
  ...
  methods: {
    ...
    computeEmbedPreviewViewBox() {
      ...
      const otherBounds = unionEmbedPreviewBounds(nodeBounds.slice(1))
      const rootCenter = centerOfEmbedPreviewBounds(rootBounds)
      const otherCenter = otherBounds
        ? centerOfEmbedPreviewBounds(otherBounds)
        : rootCenter
      const nodeVisualScale = computeEmbedPreviewNodeVisualScale(
        nodeBounds.length
      )
      const targetRootRatio =
        this.getEmbedPreviewTargetRootScreenRatio() * nodeVisualScale
      const baseWidth = Math.max(
        rootBounds.width / targetRootRatio,
        (rootBounds.height / targetRootRatio) * EMBED_PREVIEW_TARGET_ASPECT
      )
      const viewSize = {
        width: baseWidth,
        height: baseWidth / EMBED_PREVIEW_TARGET_ASPECT
      }
      const rawCenter = {
        x:
          rootCenter.x +
          (otherCenter.x - rootCenter.x) *
            EMBED_PREVIEW_CENTER_TOWARD_OTHERS_RATIO,
        y:
          rootCenter.y +
          (otherCenter.y - rootCenter.y) *
            EMBED_PREVIEW_CENTER_TOWARD_OTHERS_RATIO
      }
      const limitedCenter = {
        x: clampEmbedPreviewValue(
          rawCenter.x,
          rootCenter.x -
            viewSize.width * EMBED_PREVIEW_ROOT_CENTER_LIMIT_RATIO,
          rootCenter.x +
            viewSize.width * EMBED_PREVIEW_ROOT_CENTER_LIMIT_RATIO
        ),
        y: clampEmbedPreviewValue(
          rawCenter.y,
          rootCenter.y -
            viewSize.height * EMBED_PREVIEW_ROOT_CENTER_LIMIT_RATIO,
          rootCenter.y +
            viewSize.height * EMBED_PREVIEW_ROOT_CENTER_LIMIT_RATIO
        )
      }
