...
import {
  ...
  waitForMindMapContainerEl
} from '@/utils/mindMapConstructorOptions'
...
import { isSaveShortcut } from 'simple-mind-map/src/utils/index.js'
...
import BaseStylePanel from './BaseStyle.vue'
import ThemePanel from './Theme.vue'
import StructurePanel from './Structure.vue'

const OutlineSidebar = () => import('./OutlineSidebar.vue')
const Style = () => import('./Style.vue')
const BaseStyle = window.takeOverApp ? BaseStylePanel : () => import('./BaseStyle.vue')
const Theme = window.takeOverApp ? ThemePanel : () => import('./Theme.vue')
const Structure = window.takeOverApp
  ? StructurePanel
...
export default {
  ...
  mounted() {
    ...
    window.addEventListener('keydown', this.onHostSaveShortcut, true)
  ...
  beforeDestroy() {
    ...
    window.removeEventListener('keydown', this.onHostSaveShortcut, true)
  ...
  methods: {
    ...
    tryRevealEmbedAfterInitialViewport(hideCurrentLoading, attempt = 0) {
      let result = null
      try {
        result = this.applyEmbedFocusedViewport('initial-render-end')
      } catch (error) {
        console.error(error)
        debugMindMapOpen('applyEmbedFocusedViewport threw', {
          attempt,
    ...
    tryRevealEditorAfterInitialViewport(hideCurrentLoading, attempt = 0) {
      let result = null
      try {
        result = this.applyEditorFocusedViewport('editor-initial-render-end')
      } catch (error) {
        console.error(error)
        debugMindMapOpen('applyEditorFocusedViewport threw', {
          attempt,
          message: error && error.message ? error.message : String(error)
        })
        hideCurrentLoading()
    ...
    onHostSaveShortcut(event) {
      ...
      const textEdit = this.mindMap && this.mindMap.renderer && this.mindMap.renderer.textEdit
      if (
        textEdit &&
    ...
    async init() {
      ...
      const themeNormalized = normalizeMindMapTheme(
        theme,
        exampleData.theme.template
      )
      theme = {
        template: themeNormalized.template,
        config: compactCustomThemeConfig(
          themeNormalized.config || {},
          themeNormalized.template
        )
      }
      this.mindMapData = {
        ...(this.mindMapData || {}),
      ...
      const containerEl = await waitForMindMapContainerEl(this)
      if (!containerEl) {
        throw new Error('MindMap 容器 #mindMapContainer 未就绪')
      }
      const overlayRoot = this.$refs.mindMapOverlayRoot || null
      const persistedConfig = pickMindMapPersistedConfig(config)
      ...
      this.mindMap = new MindMap({
        ...
        viewData: view,
        ...getMindMapRuntimeOverlayOptions(overlayRoot),
    ...
    warmSidebarPanels() {
      ...
      const run = () => {
        Promise.all(loaders).then(mountPanels)
      }
