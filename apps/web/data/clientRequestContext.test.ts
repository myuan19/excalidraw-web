...
describe("clientRequestContext", () => {
  ...
  it("builds request headers with runtime debug context", async () => {
    ...
    const headers = clientRequestContext.buildClientRequestHeaders("auto-save");
  ...
  it("returns logger context using stable session ids", async () => {
    ...
    const first = clientRequestContext.buildClientRequestHeaders("getFile");
    const context = clientRequestContext.getClientLoggerContext();
    const second = clientRequestContext.buildClientRequestHeaders("save");
    ...
    expect(second["X-EditorHub-Trace-Id"]).toBe(first["X-EditorHub-Trace-Id"]);
