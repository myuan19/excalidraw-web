...
const RUN_ID_STORAGE_KEY = "editorhub-debug-run-id";
const TRACE_ID_STORAGE_KEY = "editorhub-debug-trace-id";
...
function getClientRunId(): string {
  return getSessionContextId(
    RUN_ID_STORAGE_KEY,
    "run",
    memoryRunId,
...
function getClientTraceId(): string {
  return getSessionContextId(
...
function getDebugCaseId(): string | null {
  try {
    return sessionStorage.getItem(CASE_ID_STORAGE_KEY)?.trim() || null;
  } catch {
    return null;
  }
}
