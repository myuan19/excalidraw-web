...
describe("saveQueue automatic source guards", () => {
  ...
  it("ignores idle auto saves when idle delay is off without blocking exit saves", async () => {
    updateAppSettings({ autoSaveEnabled: true, autoSaveIdleSec: 0 });
    const executor = vi.fn(async (req) => ({
      saved: true,
      fileId: `file-${req.source}`,
    }));
    cleanup = installExecutor(executor);

    await expect(requestSaveAndWait({ source: "auto" })).resolves.toEqual({
      saved: false,
    });
    expect(executor).not.toHaveBeenCalled();

    await expect(requestSaveAndWait({ source: "home" })).resolves.toEqual({
      saved: true,
      fileId: "file-home",
    });
    expect(executor).toHaveBeenCalledTimes(1);
...
describe("saveQueue remote update prompt guards", () => {
