...
export function migrateManagedDocument(raw: unknown): ManagedDocument {
  ...
  if (!document) {
    throw new Error("Cannot migrate unknown document");
  }

  if (document.containerVersion > CURRENT_CONTAINER_VERSION) {
    throw new Error(
      `Unsupported containerVersion ${document.containerVersion}`,
    );
  }
  ...
  if (document.formatVersion > currentFormatVersion) {
    throw new Error(
      `Unsupported ${document.kind} formatVersion ${document.formatVersion}`,
    );
  }
  ...
  for (
    ...
    if (!migration) {
      throw new Error(
        `Missing migration for ${document.kind} formatVersion ${version} -> ${
          version + 1
        }`,
      );
    }
