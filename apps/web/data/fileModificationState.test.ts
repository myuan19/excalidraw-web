...
describe("fileModificationState", () => {
  afterEach(() => {
    localStorage.clear();
    clearTabFileDirty("server-file-clean");
    clearTabFileDirty("server-file-dirty");
  });
  ...
  it("ignores native pending dirty hash when cache is still a mindmap template", () => {
    const fileId = `local-draft:${crypto.randomUUID()}`;
    const document = MindMapAdapter.toDocument(MindMapAdapter.createEmpty());
    FileSyncState.setBaselineHash(fileId, "baseline");
    FileSyncState.setDraftHash(fileId, "mindmap-native-dirty-pending:baseline");
    FileSyncState.setLocalCache(fileId, {
      document,
      elements: undefined,
      appState: undefined,
    ...
    expect(readStoredFileModificationState(fileId, "mindmap").modified).toBe(
      false,
    );
    expect(
      readStoredFileModificationState(fileId, "mindmap").shouldPromptOnLeave,
    ).toBe(false);
  });
  ...
  it("clears stale mindmap pending dirty when live document matches baseline", () => {
    const fileId = "server-file-clean";
    const document = MindMapAdapter.toDocument(MindMapAdapter.createEmpty());
  ...
  it("marks tab-local dirty only when the canonical state is modified", () => {
    const fileId = "server-file-dirty";
...
describe("evaluateArchiveCoverage", () => {
  ...
  it("allows restore when synced and server sha matches an archive", () => {
    ...
    const coverage = evaluateArchiveCoverage(
      fileId,
