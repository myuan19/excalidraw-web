...
import { getAppSettings, subscribeAppSettings } from "../data/appSettings";
import {
  getDebugCapability,
  loadDebugCapability,
...
export function DebugModeTrigger() {
  ...
  return (
    <button
      ...
      aria-label={
        enabled
          ? "Debug 日志已开启，点击版本号可关闭"
          : "连续点击版本号开启 Debug 日志"
      }
      title={
        enabled ? "Debug 日志已开启，点击关闭" : "连续点击 5 次开启 Debug 日志"
      }
      onClick={() => {
        if (enabled) {
    ...
    >
      ...
      {enabled ? (
        <span className="debug-mode-trigger__badge">Debug</span>
      ) : null}
    </button>
  );
}
