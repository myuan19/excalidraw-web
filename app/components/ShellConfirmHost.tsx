...
export const EDITOR_PLATFORM_CONFIRM_ROOT_ID = "editor-platform-dialog-root";
export const FILE_LIST_CONFIRM_ROOT_ID = "filelist-shell-confirm-root";

function ensureDialogRoot(rootId: string): HTMLElement {
  const existing = document.getElementById(rootId);
  if (existing) {
    return existing;
  }
  throw new Error(`${rootId} is missing`);
}

type ShellConfirmHostProps = {
  rootId: string;
};

/** 平台级确认弹窗宿主：与侧栏/切换提醒同级，覆盖编辑器 iframe 或文件列表。 */
export function ShellConfirmHost({ rootId }: ShellConfirmHostProps) {
